...
router.put('/:id', async (req, res) => {
  try {
    ...
    // Update students' class_signed_up array
    await db.collection('students').updateMany(
      { student_id: { $in: updatedClass.participants } },
      { $addToSet: { class_signed_up: updatedClass.name } }
    );

    const updatedClassData = await db.collection('classes').findOne({ _id: classId });
    res.json({ message: "Class updated successfully", class: updatedClassData });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
});
